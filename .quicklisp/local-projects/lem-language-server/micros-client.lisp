...
(defclass posix-process ()
  ((pid :accessor process-pid :initarg :pid)))
...
(defun create-server-process (port &key (swank-port nil))
  (when swank-port
    (error 'create-process-error :message "TODO: add swank support"))
  (let ((fpid (poiu/fork:posix-fork)))
    (cond
      ...
      ((= fpid 0)
       (progn
         (log:debug "Running micros create-server on port ~D" port)
         (micros:create-server :dont-close t :port port)
         nil))
      (t
       (log:debug "Micros server launched: ~D" fpid)
       (make-instance 'posix-process :pid fpid))))
  #|
  (let ((command
          `("ros"
            "run"
...
(defun receive-process-output-loop (process)
  (error "TODO: implement process receive")
  (loop
...
(defun start-server-and-connect (deny-port)
  (let* ((micros-port (random-available-port deny-port))
         ...
         (swank-port nil #| (random-available-port deny-port micros-port) |#)
         (process (create-server-process micros-port :swank-port swank-port)))
    (when process
      (log:debug "server process" (process-pid process))
      ...
      (let ((connection (connect-until-successful "localhost" micros-port)))
        ...
        ;; TODO implement all of async-process
        #| (sb-thread:make-thread (lambda ()
                                 (receive-process-output-loop process))) |#
        connection))))

(defun connect (hostname port)
  (log:info "connecting to ~A port ~D" hostname port)
...
(defun stop-server (connection)
  ...
  ;; TODO: implement child process termination via SIGTERM
