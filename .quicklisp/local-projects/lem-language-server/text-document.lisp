...
(defun buffer-text-document (buffer)
  (lem-base:buffer-value buffer 'text-document))

(defun (setf buffer-text-document) (text-document buffer)
  (setf (lem-base:buffer-value buffer 'text-document) text-document))
...
(defun close-text-document (text-document)
  ...
  (lem-base:delete-buffer (text-document-buffer text-document))
          ...
          (t
           (let ((start-position (lsp:range-start range))
             ...
             (lem-base:with-point ((start point)
               ...
               (lem-base:insert-string start text)))))))
...
(defun buffer-uri (buffer)
  (let ((text-document (buffer-text-document buffer)))
    (cond (text-document
          ...
          (t
           (assert (lem-base:buffer-temporary-p buffer))
           ;; Temporary buffer are not associated with text-document, so return URI based on filename
           (pathname-to-uri (lem-base:buffer-filename buffer))))))

(defun point-to-lsp-location (point)
  (let ((uri (buffer-uri (lem-base:point-buffer point))))
