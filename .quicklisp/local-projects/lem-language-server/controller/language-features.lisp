...
(defun buffer-package (buffer)
  (lem-base:with-point ((point (lem-base:buffer-start-point buffer)))
    (loop :when (in-package-line-p (lem-base:line-string point))
          :return :it
          :while (lem-base:line-offset point 1))))

(defun scan-current-package (point &optional (default "COMMON-LISP-USER"))
  (lem-base:with-point ((p point))
    (loop
      (let ((package-name (in-package-line-p (lem-base:line-string p))))
        (when package-name
          (return package-name)))
...
(defun definitions-at-point (point)
  (when-let* ((package-name (scan-current-package point))
              (symbol-string (lem-base:symbol-string-at-point point)))
...
(defun move-to-location-position (point location-position)
  (destructuring-ecase location-position
    ((:position position)
     (lem-base:move-to-bytes point (1+ position)))
    ((:offset start offset)
     (lem-base:move-to-position point start)
     (lem-base:character-offset point offset))
    ((:line line-number &optional column)
     (lem-base:move-to-line point line-number)
     (if column
         (lem-base:line-offset point 0 column)
         (lem-base:back-to-indentation point)))
    ((:function-name name)
     (lem-base:buffer-start point)
     (lem-base:search-forward-regexp point
  ...
  (destructuring-ecase location-buffer
    ((:file filename)
     (when (probe-file filename)
       (lem-base:find-file-buffer filename
...
(define-request (document-highlight-request "textDocument/documentHighlight")
  ...
  (let ((point (text-document-position-params-to-point params)))
    (unless (lem-base:in-string-or-comment-p point)
      (let ((symbol-string (lem-base:symbol-string-at-point point))
            (buffer (lem-base:point-buffer point)))
        (when symbol-string
          (lem-base:with-point ((point (lem-base:buffer-point buffer)))
            (lem-base:buffer-start point)
            (convert-to-json
             (coerce (loop :while (lem-base:search-forward-symbol point symbol-string)
                           :unless (lem-base:in-string-or-comment-p point)
                           :collect (lem-base:with-point ((start point))
                                      (lem-base:search-backward-symbol start symbol-string)
      ...
      (let* ((form (read-from-string doc))
             ...
             (documentation (make-markdown-documentation
                             (hover-symbol function-name
                                           (buffer-package (lem-base:point-buffer point)))))
             (*print-case* :downcase))
        (make-instance
         'lsp:signature-help
         :signatures (vector
...
(defun collect-definition-symbols-in-buffer (buffer default-package-name)
  (lem-base:with-point ((point (lem-base:buffer-point buffer)))
    (lem-base:buffer-start point)
    (loop :while (lem-base:search-forward-regexp point "^\\s*\\(def")
          :when (cond ((lem-base:in-comment-p point)
                       (lem-base:maybe-beginning-of-comment point)
                       (lem-base:skip-space-and-comment-forward point)
                       nil)
                      ((lem-base:in-string-p point)
                       (lem-base:maybe-beginning-of-string point)
                       (lem-base:form-offset point 1)
                       nil)
                      (t
                       (when (and (lem-base:form-offset point 1)
                                  (lem-base:skip-space-and-comment-forward point)
                                  (or (lem-base:syntax-symbol-char-p (lem-base:character-at point))
                             ...
                             (when symbol-name
                               (cond ((equal package-name "") nil) ; keyword
                                     ...
                                     (t
                                      (lem-base:with-point ((start point)
                                                       (end point))
                                        (lem-base:skip-symbol-forward start)
                                        (lem-base:skip-symbol-forward end)
                                        (let ((package (or package-name default-package-name)))
                                          (make-symbol-definition
                                           ...
                                           :line-string (lem-base:line-string point))))))))))))
          :collect :it)))
...
(defun insert-indent-space (point old-column new-column options)
  ...
  (lem-base:line-start point)
  (lem-base:delete-character point old-column)
  (let ((line-number (point-lsp-line-number point)))
    (cond ((lsp:formatting-options-insert-spaces options)
           (lem-base:insert-character point #\space new-column)
          ...
          (t
           (let ((tab-size (lsp:formatting-options-tab-size options)))
             (multiple-value-bind (num-tabs num-spaces) (floor new-column tab-size)
               (lem-base:insert-character point #\tab num-tabs)
               (lem-base:insert-character point #\space num-spaces)
...
(defun delete-trailing-line (point)
  (lem-base:with-point ((start point :left-inserting)
                   (end point :left-inserting))
    (lem-base:skip-whitespace-backward (lem-base:line-end start))
    (lem-base:line-end end)
    (unless (lem-base:point= start end)
...
(defun indent-line (point options)
  (check-type options lsp:formatting-options)
  (unless (lem-base:blank-line-p point)
    (let ((old-column (lem-base:point-charpos (lem-base:back-to-indentation point)))
          (new-column (lem-lisp-syntax:calc-indent (lem-base:copy-point point :temporary))))
      (when new-column
        (edit-indent-line point old-column new-column options)))))

(defun indent-lines (start end options)
  (let ((text-edits '()))
    (lem-base:apply-region-lines start
                            end
                            (lambda (point)
                              (setf text-edits
...
(defun trim-and-insert-final-newlines-edit (buffer options)
  (let ((text-edits '()))
    (lem-base:with-point ((start (lem-base:buffer-point buffer))
                     (end (lem-base:buffer-point buffer)))
      (when (handler-case (lsp:formatting-options-trim-final-newlines options)
              (unbound-slot () nil))
        (lem-base:buffer-end start)
        (lem-base:buffer-end end)
        (lem-base:skip-whitespace-backward start)
      ...
      (when (handler-case (lsp:formatting-options-insert-final-newline options)
        ...
        (lem-base:buffer-end end)
        (lem-base:skip-whitespace-backward start)
        (when (and (lem-base:point= start end)
                   ;; is buffer empty?
                   (not (lem-base:start-buffer-p start)))
          (let ((text-edit (make-instance 'lsp:text-edit
                                          :new-text (string #\newline)
                                          :range (points-to-lsp-range start end))))
            (lem-base:delete-between-points start end)
...
(defun call-with-indent-text-document (text-document function)
  (let* ((buffer (text-document-buffer text-document))
         (old-buffer-text (lem-base:buffer-text buffer)))
    (lem-base:buffer-enable-undo buffer)
    (let ((text-edits (funcall function buffer)))
      (lem-base:buffer-undo (lem-base:buffer-point buffer))
      (lem-base:buffer-disable-undo buffer)
      (assert (string= old-buffer-text (lem-base:buffer-text buffer)))
...
(defun indent-text-document (text-document options)
  (call-with-indent-text-document
   ...
   (lambda (buffer)
     (let ((text-edits
             (indent-lines (lem-base:buffer-start-point buffer)
