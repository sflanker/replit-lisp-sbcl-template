  ...
  (convert-to-json
   (make-instance
    ...
    :server-info (make-lsp-map "name" (language-server-name)
                               ...
                               "swankPort" (trace-info "swank-port" (swank-port (trace-info "current-server " (current-server))))
  ...
  (convert-to-json
   ...
   (make-instance
    ...
    :server-info (make-lsp-map "name" (language-server-name)
                               ...
                               "microsPort" (micros-port (current-server)))))
     ;; a "NO-APPLICABLE-METHOD-ERROR" is being thrown
     ;; pobably related to the haxin with regards to server process or swank port
     (error (e)
            (log:error "derp" e)
            nil)) ))
|#
